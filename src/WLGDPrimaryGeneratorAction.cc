...
void WLGDPrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  if(fGenerator == "Ge77m" || fGenerator == "Ge77andGe77m")
  {
    ...
    G4double pphi           = CLHEP::twopi * rndm(generator);
    ...
    G4double phi           = 0 * rad;
    G4double x =
      radius * cos(pphi) + roiradius * cm * std::cos(string_number * angle) + offset_x;
    G4double y =
      radius * sin(pphi) + roiradius * cm * std::sin(string_number * angle) + offset_y;
