...
void WLGDDetectorConstruction::DefineMaterials()
{
  ...
  // Sincler - Boron density and percent
  // G4double borondensity = (B_NumberRatio*10 + (1 - B_NumberRatio)*11)* g / mole; // Deveria ser assim? Sincler
  G4double borondensity = 2.34 * g / cm3; 
  G4double doppercent = 1;
  G4double doplevel = doppercent/100;
  G4double doplevelred = 1-doplevel;

  G4double densityBPMMA = 1.18*doplevelred + doplevel*borondensity;
  G4double Hfrac = 0.08 * doplevelred;
  G4double Cfrac = 0.60 * doplevelred;
  G4double Ofrac = 0.32 * doplevelred;
  ...
  BoratedPMMA->AddElement(SpecialB, SSB);

  // Sincler - Gadolinium density and percent

  G4double densityGd = 7.90 * g / cm3;
  G4double gadoppercent = 1;
  G4double gadoplevel = gadoppercent/100;
  G4double gadoplevelred = 1-doplevel;

  G4double densityGadoPMMA = 1.18*gadoplevelred + gadoplevel*densityGd;
  G4double GadoHfrac = 0.08 * gadoplevelred;
  G4double GadoCfrac = 0.60 * gadoplevelred;
  G4double GadoOfrac = 0.32 * gadoplevelred;
...
auto WLGDDetectorConstruction::SetupBaseline() -> G4VPhysicalVolume*
{
  ...
  if(fSetMaterial == "BoratedPMMA")
    BoratedPETMat = G4Material::GetMaterial("BoratedPMMA"); // Sincler
  if(fSetMaterial == "GadoPMMA")
    BoratedPETMat = G4Material::GetMaterial("GadoPMMA"); // Sincler
  if(fSetMaterial == "Gado381PMMA")
    BoratedPETMat = G4Material::GetMaterial("Gado381PMMA"); // Sincler   
...
auto WLGDDetectorConstruction::SetupHallA() -> G4VPhysicalVolume*
{
  ...
  fLayerLogical->SetVisAttributes(G4VisAttributes::GetInvisible);
...
void WLGDDetectorConstruction::DefineCommands()
{
  ...
  fDetectorMessenger
    ...
    .SetGuidance("BoratedPMMA = to use Doped Boron PMMA") // Sincler
    .SetGuidance("GadoPMMA = to use Doped Gd PMMA") // Sincler  
    .SetGuidance("Gado381PMMA = MMA-co-Gd(MAA)3 - Gd - 3,81%") // Sincler   
    .SetCandidates("BoratedPE PolyEthylene PMMA BoratedPMMA GadoPMMA Gado381PMMA") // When a material is add in WLGD entry - Sincler
